...
{ // A motorized Window Shade with Hold Feature
  ...
  const char *code = "";
  int interrupt = 0;
  int seconds = 1600; // how many seconds it takes to go 10% of the full shade distance
  int dist = 0;       // distance to go -passed between raise and lower function and loop
  ...
  DEV_WindowShade(int TXPIN) : Service::WindowCovering()
  { // constructor() method
    txPin = TXPIN;
  ...
  void do_transmit()
  {
    ...
    WEBLOG("Preamble %s Header %s Code %s TxPin %s", String(preamble_size), String(header_size), String(code_size), String(txPin)); // Debug
  ...
  boolean update()
  {
    ...
    {                                       // if the target-position requested is less than the current-position, simply log a "raise" message
  ...
  { // loop() method
    ...
    if (current->getVal() != target->getVal() && target->timeVal() > 1000)
    {                                                                                                               // If the targetvalue does not match the current value and 16 seconds ( full shade drop) then evaluate for type of position change.
